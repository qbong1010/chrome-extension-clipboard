...
//---------------- 건축물대장 조회 기능 ----------------
const API_KEY = '0432b6814606f51e00ba673c512ed8973ff859a6ed723fa5591b736c76be31fb';
...
function renderSearchResults(results) {
  ...
  if (results.length === 0) {
    addressResultsList.innerHTML = '<li class="no-results">검색 결과가 없습니다</li>';
...
function validateAndFormatInputs() {
  ...
  if (!sigunguInput.value || !bjdongInput.value || !bunInput.value) {
    throw new Error('필수 항목을 모두 입력해주세요 (시군구코드, 법정동코드, 본번)');
  }
  
  if (sigungu.length !== 5 || bjdong.length !== 5 || platGb.length !== 1 || bun.length !== 4 || ji.length !== 4) {
...
function renderDetailView(data) {
  ...
  let tableHtml = '<table><thead><tr><th>항목</th><th>값</th></tr></thead><tbody>';
